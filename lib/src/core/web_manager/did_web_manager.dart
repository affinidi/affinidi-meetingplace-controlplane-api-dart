import 'package:ssi/ssi.dart' hide DidWeb;
...
class DidWebManager extends DidManager {
  ...
  Future<void> _mapX25519KeyAgreementMethod(String walletKeyId) async {
    ...
    if (didDocument.keyAgreement.isEmpty) {
      throw SsiException(
        message: 'No key agreement methods found in did:web document.',
  ...
  Future<DidDocument> getDidDocument() async {
    ...
    final publicKeys = await Future.wait(
      keyIds.map((keyId) async => await wallet.getPublicKey(keyId)),
    );
